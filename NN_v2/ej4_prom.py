...
from utils import particionar_k_out, convert_to_one_dimension, WinnerTakesAll, particionar, Promediar
...
def main():
    ...
    for _i in range(len(particiones)):
        nn = NN([2,5,4,3], learning_rate=.2)#nn = NN([4,4,3], learning_rate=.1)
        epocas_convergencia_iteracion = nn.Train(datos_prom[particiones[_i][0]], max_epochs=100, tol_error=.1, alfa=0.5, tam_output=3)
